...
API -->|one call after all answers| LLM
LLM -->|full report + per-task feedback| API

### User flow
graph TD
Start((Старт)) --> CheckUser[Определяем User: Новый/Оплачен/Триал]
CheckUser --> Landing[Лендинг с кнопкой Try for free / Start / Pay]
Landing --> Auth[Регистрация/Вход (email, Telegram, пароль)]
...
subgraph InterviewLoop [Цикл Интервью]
    ...
    SendAnswer --> NextOrEnd{Есть ещё задачи?}
    NextOrEnd -- Да --> GetTask
    NextOrEnd -- Нет --> Feedback
end

Feedback[Один вызов LLM: фидбек по каждой задаче + итог и рекомендации]
...
6. После нажатия на кнопку Start, если юзер не авторизован, открывается модальное окно: вход (email или Telegram + пароль) или регистрация (имя, email, Telegram по желанию, пароль). Данные хранятся в своей БД.
...
8. На этапе когда юзер готов получить свою первую задачу, описываем формат: до 3 задач, ответы текстом; **между задачами ответов от LLM нет**; после последнего ответа — один общий разбор (по каждой задаче + итог).
...
11. После отправки решения — короткий лоадер сохранения; **вызов LLM только после последней задачи** — отдельный лоадер «готовим разбор».
