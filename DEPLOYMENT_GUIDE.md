...
### Бэкенд падает: не может подключиться к БД

В проекте БД настроена **без пароля** (trust): подключения из контейнеров принимаются без проверки. Если у тебя остался старый том с паролем, один раз пересоздай данные:
...
(Это удалит данные БД. Если нужно их сохранить — сначала сделай дамп.)
